...
sub usage {
 ...
 Usage: $0 --dparam <DBParam> [--root <dir>]

 writes the block latency information to a csv.gz file, taking
data from a certain timestamp onwards. Looks in the <root> directory for
...
if ( @files = glob($type."_latency-[0-9]*.csv.gz") ) {
  $file = (sort(@files))[-1];
